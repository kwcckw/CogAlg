...
def search(P_):  # cross-compare patterns within horizontal line
    ...
    # sub search recursive is not reviewed yet
    ...
    if len(P_) > 1:  # at least 2 comparands, unpack P_:
        ...
        for n, param_name in enumerate(layer0):  # loop L_, I_, D_, M_
            Ppm_, Ppd_ = search_param_(param_name, layer0[param_name])
            Ppms_[n].append(Ppm_)
            Ppds_[n].append(Ppd_)
...
def search_param_(param_name, param):
    ...
    for i, (param, L, x0) in enumerate(param_[1:], start=1):
        ...
        negL = negM = 0
        comb_M = dert.m
        j = i # to not distort the i in current loop
        
        while comb_M > 0 and j+1 < len(param_): # next param should not exceed size of param_
            j += 1
...
def merge_P_draft(P_):
    ...
    for i, __P in enumerate(P_[:-2]): # loop and stop before second last element because after second last element there wouldn't be any P instance for the merging process
        if i not in remove_index:               
            merge_value = 1 
            j= i
            while merge_value and (j+1 not in remove_index) and (j+2 < len(P_)): # continue merging process if the previous merging process is successful
                merge_value = merge_eval_draft(P_, __P, j, negL, remove_index)
                j += 2
        ...
        if i not in remove_index:
            for j, P in enumerate(P_[i + 1:], start=1):
                ...
                if (j + i) not in remove_index:
                    ...
                    if _P.M + negM > 0:  # search while net_M > ave_M * nparams or 1st P, no selection by M sign

                        negM, negL, merge_val = merge_eval_draft(P_, _P, P, i, j + i, negM, negL, remove_index)
                        if sign:
                            ...
                            negM += mP  # accumulate contiguous P miss, or all derivatives?
                            negL += _L  # accumulate distance to matching P
                    else:
...
def merge_eval_draft(P_, __P, j, negL, remove_index):  # comp to merge
