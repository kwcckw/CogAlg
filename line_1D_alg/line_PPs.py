...
def comp_sublayers(_P, P, root_m):  # if pdert.m -> if summed params m -> if positional m: mx0?
    ...
    for _sublayer, sublayer in zip(_P.sublayers, P.sublayers):
        for i, isublayer in enumerate([_sublayer, sublayer]):  # list of subsets: index 0 = _P.sublayers, index 1 = P.sublayers
            if root_m * len(isublayer) > ave_M * 5:  # won't work, first len(isublayer) = 1, need to think about it
                # index 0 = Pm, index 1 = Pd
                subDertt  = [[0, 0, 0, 0],[0, 0, 0, 0]]  # P. L, I, D, M summed within sublayer (i think we should put here, to sum across current layer subsets)
                for fPd, rdn, rng, sub_rval_Pm_, xsub_pmdertt_, sub_rval_Pd_, xsub_pddertt_ in isublayer: # current layer subsets
                    for j, sub_rval_P__ in enumerate([sub_rval_Pm_, sub_rval_Pd_]): # j index to indicate Pm | Pd
                ...
                subDertt_s[i].append(subDertt)  # 2tuple, index 0 = _P, index 1 = P
            else:
    ...
    derDertt = [[],[]]
    for (_mDert, _dDert), (mDert, dDert) in zip(subDertt_s[0], subDertt_s[1]):
    ...
    if _P.sublayers and P.sublayers:  # or pdert.sub_M + pdert.m + P.M?
        # comp sub_Ps between sub_P_s in 1st sublayer:
        _fPd, _rdn, _rng, _sub_rval_Pm_, _xsub_pmdertt_, _sub_rval_Pd_, _xsub_pddertt_ = _P.sublayers[0][0]  # 2nd [0] is the 1st and only subset
        fPd, rdn, rng, sub_rval_Pm_, xsub_pmdertt_, sub_rval_Pd_, xsub_pddertt_ = P.sublayers[0][0]

        _xsub_pdertt_t = (_xsub_pmdertt_, _xsub_pddertt_)
        xsub_pdertt_t = (xsub_pmdertt_, xsub_pddertt_)
        _sub_rval_P_t = (_sub_rval_Pm_, _sub_rval_Pd_)
        sub_rval_P_t = (sub_rval_Pm_, sub_rval_Pd_)

        for xDert_mt, _xsub_pdertt_,  xsub_pdertt_, _sub_rval_P__, sub_rval_P__ in \
        zip(xDert_mt, _xsub_pdertt_t, xsub_pdertt_t, _sub_rval_P_t, sub_rval_P_t): # shouldn't this need to be per P, | Pd too?
            # if same intra_comp fork, else not comparable:
            if fPd == _fPd and rng == _rng and min(_P.L, P.L) > ave_Ls and root_m > 0 and  xDert_mt + root_m > ave_M * 4 :  # or pdert.sub_M + pdert.m + P.M?
                ...
                for _sub_rval_P_ in _sub_rval_P__:
                    for sub_rval_P_ in sub_rval_P__:
                    
                        for _rval, _sub_P in _sub_rval_P_[1]:  # form xsub_pdertt_[ xsub_dertt [ xsub_pdert_[ sub_pdert]]]: each bracket is level of nesting
                            P_ = []  # to form xsub_Pps
                            _xsub_pdertt = [[], [], [], []]  # tuple of L, I, D, M xsub_pderts
                            _SL += _sub_P.L  # ix0 of next _sub_P
                            # search right:
                            for rval, sub_P in sub_rval_P_[1][start_index:]:  # index_ix0 > _ix0, only sub_Ps at proximate relative positions in sub_P_ are compared
                            ...
                            # search left:
                            for rval, sub_P in reversed(sub_rval_P_[1][len(sub_rval_P_[1]) - start_index:]):  # index_ix0 <= _ix0, invert positions of sub_P and _sub_P:
