        ...
        for param_name in ["I_", "D_", "M_"]:
            ...
            if ((param_name == "I_") and not fPd) or ((param_name == "D_") and fPd):  # dert-level P-defining params
                if not fPd:
                    ...
                    Pdert__ += [ search_param_(_par_, par_, P_, ave, rave=1) ]  # pdert_ if "I_"
                del _P
                _rL_=[]
...
def search_param_(_param_, param_, P_, ave, rave):  # variable-range search in mdert_, only if param is core param?
    ...
    for i, ((_param, _L, _x0), (param, L, x0), P) in enumerate( zip(_param_, param_, P_[:-1])):

        dert = comp_param(_param, param, "I_", ave)  # param is compared to prior-P _param
        comp_sublayers(P_[i], P_[i+1], dert.m, dert.d) # place after comp_param? 
