    ...
    if len(P_) > 1:  # at least 2 comparands
        ...
        for i, P in enumerate(P_):
            if "_P" in locals():  # not the 1st P
                ...
                Ldert_.append(Cpdert(i=L, p=L+_L, d=rL, m=mL, ix0=P.x0, x0=i, L=L, _P=P))
                rL_.append(rL)
            _P = P
        ...
        for param_name, pdert_, rdn_ in zip(layer0, pdert__, rdn__):
            ...
            Ppm_ = form_match_connected_Pp_(pdert_, P_, param_name, rdn_, fPd=0)  # Ppd_ formed in -Ppms only, in intra_Ppm_
...
def search_param_(_param_, param_, P_, param_name):  # variable-range search in mdert_, if param is core param?
    ...
    for i, ((_param, _L, _x0), (param, L, x0), P) in enumerate( zip(_param_, param_, P_)):
        ...
        mdert_.append(Cpdert(i=dert.i, p=dert.p, d=dert.d, m=dert.m, x0=i, ix0=x0, L=L, _P=P, negiL=negiL, negL=negL, negM=negM))
...
# draft and tentative
def form_match_connected_Pp_(dert_, P_, param_name, rdn_, fPd):  
    ...
    for dert, rdn in zip(dert_[1:], rdn_[1:]):  # segment by sign
        
        # match connected Pderts
        if (_dert.m and dert.m) and  (P_[_dert.x0 + _dert.negL] is dert._P): # or how to evaluate match-connected pderts?
            # accumulate params:
            Pp.L += 1; Pp.iL += dert.L; Pp.I += dert.p; Pp.D += dert.d; Pp.M += dert.m; Pp.Rdn += rdn; Pp.negiL += dert.negiL
            Pp.negL += dert.negL; Pp.negM += dert.negM
            Pp.pdert_ += [dert]
