...
def stack_2_PP_(stack_, PP_):
    ...
    for i, stack in enumerate(stack_):  # bottom-up to follow upconnects
        ...
        if stack.downconnect_cnt == 0:  # check only the root stack (would not be the upconnect of any other stack)
        ...
        if not upconnect_[0].sstack_:  # sstack_ is empty until stack is scanned over
            ...
            for dert_P in upconnect_[0].Py_:
                if (_dert_P.Pm > 0) != (dert_P.Pm > 0):
                    ...
                    dert_P.sstack.downconnect_cnt += 1 # increase upconnect' dert_P's sstack downconnect count
                ...
                for dert_P in upconnect.Py_:
                    if (_dert_P.Pm > 0) != (dert_P.Pm > 0):
                        ...
                        if not isinstance(dert_P.sstack, CSstack): # if there is no sstack created for dert_P yet
                           dert_P.sstack = CSstack(dert_Pi=dert_P)
                        dert_P.sstack.downconnect_cnt += 1 # increase upconnect' dert_P's sstack downconnect count
                        sstack.upconnect_.append(dert_P.sstack) # add upconnect' dert_P's sstack to isstack's upconnect_
                    ...
                    # i think the section below shouldn't be in else loop, since we only append the upconnect just before the sstack termination?
                    '''
