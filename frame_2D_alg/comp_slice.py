...
def comp_slice_root(blob, verbose=False):  # always angle blob, composite dert core param is v_g + iv_ga
    ...
    for dir_blob in blob.dir_blobs:  # same-direction blob should be CBlob
        ...
        for PP_ in (PPm_, PPd_):  # 1st agglomerative recursion is per PP, appending PP.seg_levels, not blob.levels:
            for PP in PP_:
                if len(PP.seg_levels[0][-1])>1: 
                    agg_recursion(PP.seg_levels, level=1, fseg=1, fiPd=0)  # comp_seg -> segPs.. per seg__[n], in PP.seg_levels
                if len(PP.seg_levels[1][-1])>1: 
                    agg_recursion(PP.seg_levels, level=1, fseg=1, fiPd=1)

        dir_blob.levels = [[PPm_], [PPd_]]
        if len(PPm_)>1: 
            agg_recursion(dir_blob.levels, level=1, fseg=0, fiPd=0)  # 2nd call per dir_blob.PP_s formed in 1st call, forms PPP..s and dir_blob.levels
        if len(PPd_)>1: 
            agg_recursion(dir_blob.levels, level=1, fseg=0, fiPd=1)  # also test for n_extended?
