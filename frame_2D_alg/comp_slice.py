...
def comp_slice_root(blob, verbose=False):  # always angle blob, composite dert core param is v_g + iv_ga
    ...
    for dir_blob in blob.dir_blobs:
        ...
        dir_blob.PPm_ = PPm_; dir_blob.PPd_ = PPd_
        ...
        dir_blob.dlayers = sub_recursion_eval(PPd_, fd=1)  # add rlayers, dlayers, seg_levels to select PPs
         ...
         # not sure if this is a right place for it: (i think it should be here, because we need CPP for sub_recursion)
        dir_blob = CBlob2Cgraph(dir_blob, fseg=0)
...
def agg_recursion_eval(dir_blob, gPP_t, fseg):
    ...
    M, G = dir_blob.valt  # dir_blob is Cgraph now
    fork_rdnt = [1+(G>M), 1+(M>=G)]

    # cross PP:
    for fd, gPP_ in enumerate(gPP_t):
        if (dir_blob.valt[fd] > PP_aves[fd] * ave_agg * (dir_blob.rdn+1) * fork_rdnt[fd]) and len(gPP_) > ave_nsub and dir_blob.alt_rdn < ave_overlap:
            dir_blob.rdn += 1  # estimate
            agg_recursion(dir_blob, gPP_, rng=2, fseg=fseg)


# rough draft:
def CBlob2Cgraph(dir_blob, fseg):
    from agg_recursion import Cgraph
    
    # retrieve PPms and PPds
...
def sub_recursion_eval(PP_, fd):  # for PP or dir_blob
    ...
    for PP in PP_:
        ...
        # convert to Cgraph
        root = CPP2Cgraph(PP, fseg=1, Cgraph=Cgraph)
        gPPm_ = [CPP2Cgraph(seg, fseg=1, Cgraph=Cgraph) for seg in PP.mseg_levels[-1]]
        gPPd_ = [CPP2Cgraph(seg, fseg=1, Cgraph=Cgraph) for seg in PP.dseg_levels[-1]]
