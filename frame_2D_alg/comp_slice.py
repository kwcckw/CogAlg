...
def convert_to_dP(_P,P, derLay, angle, distance, Et):
    ...
    if isinstance(_P, CP): link.L = min(_P.latuple[4],P.latuple[4])
    else:                  link.L = min(_P.L,P.L)  # P is CdP
    return link
...
def sum2PP(P_, dP_, Et):  # sum links in Ps and Ps in PP
    ...
    if dP_:  # add uplinks:
        ...
        S,A = P_[0].latuple[4:] if isinstance(P_[0], CP) else (P_[0].angle, P_[0].span)  # [I, G, M, D, L, (Dy, Dx)]
