    ...
    for y, _P_ in enumerate(P__[:-rng]):  # higher compared row, skip last rng: no lower comparand rows
        for x, _P in enumerate(_P_):
            for derP in _P.downlink_layers[-1]:  # lower comparands are linked Ps at dy = rng
                ...
                uplinks__[y][up_x] += [derP]  # should be just y, because uplinks are initialized with P__[rng:] 
...
def sum2PP(PP_segs, fPd, miss_uplink_, miss_downlink_):  # sum params: derPs into segment or segs into PP

    PP = CPP(x0=PP_segs[0].x0, sign=PP_segs[0].sign,L= len(PP_segs), uplink_layers = [miss_uplink_], downlink_layers = [miss_downlink_] )
        ...
        for seg in seg_:  # bottom-up
            if not isinstance(seg.root, CPP):  # seg is not already in PP initiated by some prior seg
                ...
                if seg.P__[-1].uplink_layers[-1] or seg.P__[0].downlink_layers[-1]:
                    form_PP_(PP_, [seg], seg.P__[-1].uplink_layers[-1].copy(), seg.P__[0].downlink_layers[-1].copy(), seg.uplink_layers[-1].copy(), seg.downlink_layers[-1].copy(), fPd)
                else:
                    PP_ += [sum2PP([seg], fPd, seg.uplink_layers[-1], seg.downlink_layers[-1])]  # single-seg PP
...
def form_PP_(PP_, PP_segs, uplink_, downlink_, miss_uplink_, miss_downlink_, fPd):
    ...
    for upderP in uplink_:
        if isinstance(upderP._P.root, CPP):  # highest row doesn't have _P's root, so we need this check here
            ...
            # add missing links  
            for miss_uplink in upderP._P.root.uplink_layers[-1]:
                if miss_uplink not in miss_uplink_:
                    miss_uplink_.append(miss_uplink)
            for miss_downlink in upderP._P.root.downlink_layers[-1]:
                if miss_downlink not in miss_downlink_:
                    miss_downlink_.append(miss_downlink)
    ...
    for downderP in downlink_:
        if isinstance(downderP.P.root, CPP):  # lowest row doesn't have P's root
            ...
            # add missing links
            for miss_uplink in downderP.P.root.uplink_layers[-1]:
                if miss_uplink not in miss_uplink_:
                    miss_uplink_.append(miss_uplink)
            for miss_downlink in downderP.P.root.downlink_layers[-1]:
                if miss_downlink not in miss_downlink_:
                    miss_downlink_.append(miss_downlink)
    ...
    if match_uuplink_ or match_ddownlink_:  # recursive compare sign of next-layer uplinks
        ...
        PP_ += [sum2PP(PP_segs, fPd, miss_uplink_, miss_downlink_)]  # PP_segs is converted to PP
        '''
