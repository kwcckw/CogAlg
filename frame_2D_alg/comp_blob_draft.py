...
def cross_comp_blobs(frame):
    ...
    for blob in blob_:
        ...
        if blob.id not in checked_ids_:
            ...
            comp_blob_recursive(blob, blob.adj_blobs[0], checked_ids_, net_M, compared_blobs)  # comp between blob, blob.adj_blobs
            a = 1
        compared_blob_.append(compared_blobs)

    bblob_ = form_bblob_(blob_, compared_blob_)

    visualize_cluster_(blob_,frame)
    
    return bblob_

# darft, to visualize the cluster of blobs
...
def form_bblob_(blob_, compared_blob_):
    ...
    for blob, compared_blobs in zip(blob_, compared_blob_):
        if blob.derBlob_ and blob.id not in checked_ids:  # blob is initial center of adj_blobs cluster
            ...
            for compared_blob in compared_blobs:  # depth first - check potential element from adjacent cluster of blobs
...
def comp_blob_recursive(blob, adj_blob_, checked_ids_, net_M, compared_blobs):
    ...
    for adj_blob in adj_blob_:
        if adj_blob.id not in checked_ids_ and not adj_blob.derBlob_:
            ...
            # use abs M? Since M shouldn't be signed right? And negative and postive M will cancel out each other during the net M accumulation
            net_M += abs(adj_blob.Dert.M)            # cost of extending the cross comparison
