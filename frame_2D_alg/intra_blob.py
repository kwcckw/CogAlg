        ...
        if blob_height > 3 and blob_width > 3:  # min blob dimensions: Ly, Lx
            if root_blob.fBa:
                ...
                if blob.G < aveB*blob.rdn:  # below-average G, eval for comp_r
                    ...
                    # this evaluation should be checked before cluster_fork?
                    if new_mask__.shape[0] > 2 and new_mask__.shape[1] > 2 and False in new_mask__:  # min Ly and Lx, dert__>=1
                        cluster_fork(blob, new_dert__, sign__, new_mask__, verbose, render, fBa)
                        # merge both forks' sub-blobs
                        spliced_layers = [spliced_layer + rsublayer for spliced_layer, rsublayer in
                                      zip_longest(spliced_layers, blob.rsublayers, fillvalue=[])]
                ...
                if blob.G > aveB*aveBa * blob.rdn:  # above-average G, eval for comp_a
                    ...
                    if new_mask__.shape[0] > 2 and new_mask__.shape[1] > 2 and False in new_mask__:  # min Ly and Lx, dert__>=1
                        cluster_fork(blob, new_dert__, sign__, new_mask__, verbose, render, fBa)
                        # merge both forks' sub-blobs
                        spliced_layers = [spliced_layer + asublayer for spliced_layer, asublayer in
                                      zip_longest(spliced_layers, blob.asublayers, fillvalue=[])]
            '''
...
def cluster_fork(blob, new_dert__, sign__, new_mask__, verbose, render, fBa):
    ...
    sublayers += [sub_blobs]  # 1st r|a type specific blobs
    # add deeper layers sub-blobs (mixing of different forks' sub-blobs)
