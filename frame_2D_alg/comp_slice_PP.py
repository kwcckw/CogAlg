...
def comp_slice_root(blob, verbose=False):  # always angle blob, composite dert core param is v_g + iv_ga
    ...
    for dir_blob in blob.dir_blobs:  # dir_blob should be Cblob, splice PPs across dir_blobs?
        ...
        comp_rng_root([], PPm_)  # rng+ comp_P in PPms, -> param_layer, form sub_PPs
        comp_der_root([], PPd_)  # der+ comp_P in PPds, -> param_layer, form sub_PPs
        
        sub_recursion(PPm_, PPd_)  
...
def accum_top_layer(top_layer, der_layer, start):
 ...
 # draft
def comp_rng_root(root_sublayers, PP_):
    ...
    for PP in PP_:  # PP is generic higher-composition pattern, P is generic lower-composition pattern  
        ...
        # at least 2 PPs for comparison
        if len(sub_PPm_)>1: comp_rng_root(PP.sublayers, sub_PPm_)  # rng+ comp_P in PPms, -> param_layer, form sub_PPs
        if len(sub_PPd_)>1: comp_der_root(PP.sublayers, sub_PPd_)  # der+ comp_P in PPds, -> param_layer, form sub_PPs
            ...
            for derP in PP.derP_:
                if derP.P.downconnect_cnt == 0:  # lowest derP in PP, else it was scanned in lower derP.P.upconnect_
                    for _derP in derP._P.upconnect_:
                        ...
                        # this section may need further discussion
...
def comp_der_root(root_sublayers, PP_):
