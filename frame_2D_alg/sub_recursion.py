...
def blob2graph(blob, fseg):
    ...
    mblob = Cgraph(inder_=[mpplayers], alt_Graph=alt_mblob, rng=PPm_[0].rng, rdn=blob.rdn, x0=(x0+xn)/2, xn=(xn-x0)/2, y0=(y0+yn)/2, yn=(yn-y0)/2)
    dblob = Cgraph(inder_=[dpplayers], alt_Graph=alt_dblob, rng=PPd_[0].rng, rdn=blob.rdn, x0=(x0+xn)/2, xn=(xn-x0)/2, y0=(y0+yn)/2, yn=(yn-y0)/2)
...
def PP2graph(PP, fseg, ifd=1):
    ...
    alt_Graph = Cgraph(val=alt_pplayers.val,inder_=[alt_pplayers],x0=(x0+xn)/2, xn=(xn-x0)/2, y0=(y0+yn)/2, yn=(yn-y0)/2)
