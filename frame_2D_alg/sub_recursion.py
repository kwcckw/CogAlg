...
def CPP2graph(PP, fseg, Cgraph):
    ...
    # i think fds should be based on last fds?
    plevels = CpH(H=[players], val=players.val, fds=[players.fds[-1]])  # init fd is der+?
    alt_plevels = CpH(H=[alt_players], val=alt_players.val, fds=[alt_players.fds[-1] if alt_players.fds else [0]])
