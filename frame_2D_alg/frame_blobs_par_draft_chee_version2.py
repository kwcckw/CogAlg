...
def check_rims(blob):
    ...
    if blob:
        while blob.dert_open_: 
                ...
                for dert_rim in dert.dert_rims: 
                    if dert_rim: # not empty rim
                        ...
                        if dert.sign == dert_rim.sign:   
                            ...
                            dert_rim.blob_id = dert.blob_id # update rim' blob's id
                            dert_open_.append(dert_rim) # add rim dert into open dert list
            ...
            blob.dert_closed_.append(dert)
        
        # add list of open derts to blob's open dert list
        blob.dert_open_ += dert_open_
...
def get_rim_dert(dert_,height,width):
    ...
    for y in range(height):
        for x in range(width):
            # topleft
            if y-1>=0 and x-1>=0: dert_topleft = dert__[y-1][x-1]
            else: dert_topleft = []
            # top
            if y-1>=0: dert_top = dert__[y-1][x]   
            else: dert_top = []
            # topright
            if y-1>=0 and x+1<=width-1: dert_topright = dert__[y-1][x+1]
            else: dert_topright = []            
            # right
            if x+1<=width-1: dert_right = dert__[y][x+1] 
            else: dert_right = []
            # botright
            if y+1<=height-1 and x+1<=width-1: dert_botright = dert__[y+1][x+1]
            else: dert_botright = []    
            # bot
            if y+1<=height-1: dert_bot = dert__[y+1][x] 
            else: dert_bot = []
            # botleft
            if y+1<=height-1 and x-1>=0: dert_botleft = dert__[y+1][x-1]
            else: dert_botleft = []             
            # left
            if x-1>=0: dert_left = dert__[y][x-1]
            else: dert_left = []             
...
def one_cycle_operation(pool,blob_,height,width):
    ...
    blob_ = pool.starmap(check_rims, zip(blob_)) # check and add rim derts to blob
...
def frame_blobs_parallel(dert__):
    ...
    ## 1st cycle ##
    blob_,id_map__ =  one_cycle_operation(pool, blob_,height,width)
...
if __name__ == '__main__':
    ...
    print('total blob formed = '+str(len(np.unique(id_map__))))
