...
class CN(CBase):
    ...
    def __init__(n, **kwargs):
        ...
        n.N_, n.B_, n.C_, n.L_, n.R_ = kwargs.get('N_',[]), kwargs.get('B_',[]), kwargs.get('C_',[]), kwargs.get('L_',[]), kwargs.get('R_',[])  # base elements
...
def cross_comp(root, rc, fC=0):  # rng+ and der+ cross-comp and clustering, fT: convert return to tuple
    ...
    rc = max(root.rc, rc)  # updated (get the max between root.rc and input rc? Because we incremented input rc as well, or update root.rc with input rc?)
...
def form_B__(G):  # assign boundary / background per node from Bt, no root update?
    ...
    for bG in Bt.N_:  # add R_ per boundary graph, in Fg?
        ...
        bG.R_=sorted(R_+[G], key=lambda x:(x.m/x.c), reverse=True)  # Nt.N_ = R_ if Nt.nest=0
    def R(L):
    ...
    for N in G.N_:
        ...
        for L in N.B_:
            ...
            if bG:
                if N not in bG.R_: 
                    bG.R_ += [N]  # reciprocal core
                    bG.R_=sorted(bG.R_, key=lambda x:(x.m/x.c), reverse=True)  # we need resort after adding new N?
...
def cluster_C(E_, root, rc):  # form centroids by clustering exemplar surround via rims of new member nodes, within root
    ...
    while True:
        ...
        for _C,_m,_o in sorted(_Ct_, key=lambda t: t[1]/t[2], reverse=True):
            if _m > Ave * _o:
                ...
                DTT += dTT
                if M > Ave * len(_N_) * O:
                    for n, mo in zip(_N_,mo_): n.mo_+=[mo]; n.R_+=[C]; C.Nt.N_+=[n]  # reciprocal root assign  (only n.R_ is C, while C.Nt.N_ is n) (but this R_ may conflict with edge's R_ in cross_comp later?)
    ...
    if C_:
        ...
        if val_(DTT, rc+olp,1, (len(C_)-1)*Lw, _TT=root.dTT) > 0:
            Ct = sum_N_(C_,rc,root); cross_comp(Ct, rc, fC=1)  # distant Cs, different attr weights?
            root.Ct = Ct; root.C_ =C_
    return up
...
def add_N(N, n, fC=0, froot=0):  # rn = n.n / mean.n
    ...
    if froot:
        N.N_ += [n]  # if N is root of n, we should append?
    else:  # merge
