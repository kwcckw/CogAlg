...
def cross_comp(root, rc, ifcon=1, fL=0):  # core function, mediates rng+ and der+ cross-comp and clustering, rc = rdn + olp

    iN_,L_,TT,c,TTd,cd = comp_N_((root.N_,root.B_)[fL], rc) if ifcon else comp_C_(root.N_,rc); nG_=[]  # nodes | centroids
    if L_:
        ...
        if V > 0 or fL:  # default for links?
            ...
            if V > ave * (centw, connw)[fcon] or fL:  # conn|cent clustering if dm/ddist, expensive, avd is placeholder
                if fcon:
                    nG_,rc = cluster_N(root, L_,rc, fL,ifcon)  # forms Bt, sub+ in sum2G
...
def cluster_N(root, iL_, rc, fL=0, fcon=1):  # flood-fill node | link clusters, flat
        ...
        for tL_, nF in (tN_,'Nt'), (tB_,'Bt'), (tC_,'Ct'):
            if tL_:
                ...
                if val_(Ft.dTT, rc, TTw(root), (len(Ft.N_)-1)*Lw) > 0:
                    cross_comp(Ft, rc, ifcon=mdecay(tL_ > avd))  # unlikely, doesn't add rc? (use tL_ eval for fcon?)
            ...
            if N_:
                ...
                if val_(tt, rc, TTw(root), (len(N_)-1)*Lw) > 0 or fL:
                    ...
                    if not fL and G.Bt and G.Bt.d > avd * rc * nw:  # no ddfork
                        lg_,_rc = cross_comp(G, rc, fL=1, ifcon = mdecay(G.L_) > avd)  # proximity-prior comp B_
    ...
    N_,L_ = [],[]  # add pL_, pN_ for pre-links?
    if fcon > 1: merge_C_(N_, L_) # merge similar Cs, not dCs, no recomp
    else:        L_ = iL_
    N_ = list(set([n for L in L_ for n in L.nt] + N_)); G_ = []  # include merged Cs
    if N_ and (val_(root.dTT, rc+connw, TTw(root), mw=(len(N_)-1)*Lw) > 0 or fcon>1):
        G_,rc = cluster_(root, N_,rc, fL)  # contig in feature space if centroids, no B_,C_?
        if G_:  # top root.N_
    ...
    if E_ and val_(np.sum([g.dTT for g in E_],axis=0), rc+centw, TTw(root), (len(E_)-1)*Lw) > 0:
        ...
        if C_:
            ...
            if val_(DTT, rc+olp, TTw(root), (len(C_)-1)*Lw) > 0:
                ...
                # fcon must be > 0 here?
                _,rc = cross_comp(Ct, rc, ifcon=2)  # distant Cs, different attr weights, no cross_comp dCs?
...
def sum2G(Ft_, rc, root=None, init=1, fsub=1):  # updates root if not init
    ...
    if fsub:
        ...
        if subV * ((len(N_)-1)*Lw) > ave * (rc+1) * connw:
            ...
            _N_,_rc = cluster_C(G, L_, rc+1)  # same nodes with higher filter  (links always cluster_C?)
            if _N_ and subV * ((len(_N_)-1)*Lw) > ave * _rc * nw:
                cross_comp(G,_rc, ifcon=mdecay(G.L_) > avd)  # forms own B_,Bt
    return G
