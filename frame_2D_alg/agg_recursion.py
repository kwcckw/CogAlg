    ...
    for blob in blob_:
        if not blob.sign and blob.G > aveB * blob.root.olp:
            ...
            if edge.G*((len(edge.P_)-1)*Lw) > ave * sum([P.latuple[4] for P in edge.P_]):
                ...
                Et = edge.Et
                # if Et[0] *((len(edge.node_)-1)*(edge.rng+1) *Lw) > ave * Et[2] * clust_w:  # + eval edge.link_? (We probaby need to eval dfork in cluster_edge?)
                if edge.node_: edge.node_ = [PP2G(PP,frame) for PP in edge.node_]
                if edge.link_: edge.link_ = [PP2G(PP,frame) for PP in edge.link_]
                Ft_Lay = cluster_edge(edge, frame)
                if Ft_Lay[0]: edge_ += [Ft_Lay]  # 1layer derH, alt: converted adj_blobs | alt_P_?
    ...
    for fi in 1,0:
        if edge.Et[1-fi] *((len(edge.node_ if fi else edge.link_)-1)*(edge.rng+1) *Lw) > ave * edge.Et[3-fi] * clust_w: 
            ...
            if val_(mEt, mEt, ave, fi=1) > 0:
                ...
                fork = [PP_, Gm_]  # (ppm_,gm_) or (ppd_,gd_)
            else: fork = [[],[]]
            if fi:  # der+ is not recursive
                if val_(dEt, dEt, ave, fi=0) > 0:
                    dL_, _, _, _, _ = comp_PP_(L2N(dL_))  # to add rimt?
                    Gd_ = cluster_L_(None, dL_, ave, rc=2) if dEt[0] * (len(dL_)-1) * Lw > ave * dEt[2] * clust_w else []
                    fork = [fork, Gd_]  # ((ppm_,gm_),Lppm_)
                else: fork = [fork, []]  # preserve index for unpacking later 
            Ft += [fork]  # we need bracket here to pack them as new level?
...
def cluster_L_(root, L_, ave, rc):  # CC links via direct llinks, no dist-nesting
    ...
    for L in L_:
        ...
        for lL, _ in L.rimt[0] + L.rimt[1]:
            ...
            if _L not in L_: continue  # _L may not in L_ (when eval of if m > ave * n * loop_w: is false in comp_PP_)
            if not _L.fin:
                _L.fin = 1; node_ += [_L]
        if val_(Et, Et, ave*clust_w) > 0:
            Lay = CLay()
