...
def form_graph_(PP_, rng, fseg):
    ...
    mnode_, dnode_ = comp_PP_(PP_, rng, fseg)
    # cross-comp all PPs within rng (PPs may be segs), compose graphs from +ve PP links

    graph_t = []
    for fd, igraph_ in enumerate([mnode_, dnode_]):
        graph_ = copy(igraph_)  # copy for popping
        while graph_:
            graph, link_valt = graph_.pop(0)  # eval intermediate nodes to extend / prune / merge graphs:
            node_, valt = graph
            cluster_node_layer(graph_= igraph_, graph=graph, node_ = node_, shared_Val=valt[fd] + link_valt[fd], fid=fd)  # node_=graph[0], valt=graph[1]
...
#                    valt = [sum([mtuple.val for plevel in mplevel[0] for players in plevel[0] for mtuple in player]),
    ...
    for node in node_:
        for (PP, derPP, fint) in node.link_:
            ...
            for (_PP, _derPP, _fint) in PP.link_:
                if _PP is not PP:  # with current scheme, this will never be true?
...
def sum2graph_(igraph_, fd):  # sum nodes' params into graph
    ...
    for igraph in igraph_:
        node_, link_valt = igraph[0]
        # draft:
        graph = Cgraph(node_=node_, plevels = [[],[],[0,0]], valt=link_valt, fds=deepcopy(node_[0].fds), x0=node_[0].x0, xn=node_[0].xn, y0=node_[0].y0, yn=node_[0].yn)
        for node in node_:
