...
def agg_recursion(root, G_, fseg, ifd):  # compositional recursion in root.PP_, pretty sure we still need fseg, process should be different
    ...
    for fd, graph_ in enumerate([mgraph_, dgraph_]):
        if fd: root_val = root.dplevels.val
        else:  root_val = root.mplevels.val
    ...
    for i, _G in enumerate(G_):
        for G in G_[i+1:]:
            ...
            if G in [node for link in _G.link_ for node in link.node_]:  # G,_G was compared in prior rng+, add frng to skip?
                continue
            if ifd: _G_plevels = _G.dplevels; G_plevels = G.dplevels
            else:   _G_plevels = _G.mplevels; G_plevels = G.mplevels
            ...
            if distance < ave_distance * ((_G_plevels.val+G_plevels.val) / (2*sum(G_aves))):
                # comb G eval
                ...
                derG = Cgraph( node_=[_G,G], mplevels=mplevel, dplevels=dplevel, val=mplevel.val + dplevel.val, y0=y0, x0=x0,
...
def eval_med_layer(graph_, graph, fd):  # recursive eval of reciprocal links from increasingly mediated nodes
    ...
    for mG, dir_mderG, G in medG_:  # assign G and shortest direct derG to each med node?
        if not mG.roott[fd]:  # root wasn't remove before
            continue
        if fd: G_plevels = G.dplevels
        else:  G_plevels = G.mplevels 
        ...
        for mderG in mG.link_:  # all evaluated links
            ...
            for derG in G.link_:
                ...
                if mmG is try_mG:  # mmG is directly linked to G
                    if fd: plevels_S = derG.dplevels.S; dir_med_plevels_S = dir_mderG.dplevels.S
                    else:  plevels_S = derG.mplevels.S; dir_med_plevels_S = dir_mderG.mplevels.S
                    if plevels_S > dir_med_plevels_S:
            ...
            if fmed:  # mderG is not reciprocal, else explore try_mG links in the rest of medG_
                for mmderG in mmG.link_:
                    if G in mmderG.node_:  # mmG mediates between mG and G
                        if fd: adj_val = mmderG.dplevels.val - ave_agg
                        else:  adj_val = mmderG.mplevels.val - ave_agg  # or increase ave per mediation depth
                        # adjust nodes:
                        if fd: G_plevels.val += adj_val; mG.dplevels.val += adj_val  # valts are not updated
                        else:  G_plevels.val += adj_val; mG.mplevels.val += adj_val
        ...
        if _graph in graph_ and _graph is not graph:  # was not removed or merged via prior _G
            ...
            for _node, _medG in zip(_node_, _medG_):  # merge graphs, add direct links:
                ...
                for _derG in _node.link_:
                    if fd: adj_Val += _derG.dplevels.val - ave_agg
                    else:  adj_Val += _derG.mplevels.val - ave_agg
...
#                    if link.dval>0 and link not in node_:
        ...
        valSub = graph_plevels.val  # last plevel valt: ca = dm if fd else md: edge or core, or single player?
...
def sum2graph_(G_, fd):  # sum node and link params into graph, plevel in agg+ or player in sub+: if fderG?
    ...
    for G in G_:
        ...
        for node in node_:  # 2nd pass defines max distance and other params:
            ...
            if fd: graph_plevels = graph.dplevels; node_plevels = node.dplevels  # same for fder
            ...
            for derG in node.link_:
                if fd: derG_plevels = derG.dplevels
                else:  derG_plevels = derG.mplevels
                ...
                for G in derG.node_:
                    if G not in graph.node_:  # alt graphs are roots of not-in-graph G in derG.node_
                        ...
                        if alt_graph not in graph.alt_graph_ and isinstance(alt_graph, Cgraph):  # not proto-graph or removed
                            if fd: alt_graph_plevels = alt_graph.dplevels
                            else:  alt_graph_plevels = alt_graph.mplevels
                            if AltTop.H:  # der+: plevels[-1] += player, rng+: players[-1] = player?
                                sum_pH(AltTop.H[0], alt_graph_plevels.H[0])
                            else:
                                AltTop.H = [alt_graph_plevels.H[0]]; AltTop.val = alt_graph_plevels.H[0].val; AltTop.fds = alt_graph_plevels.H[0].fds
