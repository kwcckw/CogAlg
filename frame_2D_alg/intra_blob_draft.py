...
def intra_blob_root(root_blob, render, verbose):  # slice_blob or recursive input rng+ | angle cross-comp within input blob
    ...
    for i, blob in enumerate(blob_):  # print('Processing blob number ' + str(bcount))
        ...
        if blob_height > 3 and blob_width > 3:  # min blob dimensions
            fdeep = 0  # flag: blob is extended (should be reinitialized per blob)
            if blob.M > 0:
            ...
            if blob.fBa:  # p fork:
                ...
                if -sub_blob.M > AveB:  # replace with borrow_M when known
                    ...
                    if mask__.shape[0] > 2 and mask__.shape[1] > 2 and False in mask__:  # min size in y and x, least one dert in dert__
                        ...
                        cluster_sub_eval(sub_blob, adert__, sign__, mask__, render, verbose)  # forms sub_blobs of fork p sign in unmasked area
                        # not sure on this yet， tentative draft to add deeper sublayers to blob's sublayers
                    ...
                    if sub_blob.M > AveB:  # comp_r fork
                        ...
                        if mask__.shape[0] > 2 and mask__.shape[1] > 2 and False in mask__:  # min size in y and x, at least one dert in dert__
                            ...
                            cluster_sub_eval(sub_blob, dert__, sign__, mask__, render, verbose)  # forms sub_blobs of sign in unmasked area
                            # not sure on this yet， tentative draft to add deeper sublayers to blob's sublayers
                            blob.sublayers += [subsublayers + sublayers for subsublayers, sublayers in
                                              zip_longest(sub_blob.sublayers, blob.sublayers, fillvalue=[])]
