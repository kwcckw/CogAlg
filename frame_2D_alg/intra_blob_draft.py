    ...
    for sub_blob in blob['blob_']:  # eval intra_blob comp_g | comp_rng if low gradient
        if sub_blob['sign']:
            if sub_blob['Dert']['M'] > aveB * rdn:  # -> comp_r:
                blobs = intra_blob(sub_blob, rdn + 1, rng**2, fig=fig, fcr=1)  # rng=1 in first call
                sub_blob = blobs
        elif sub_blob['Dert']['G'] > aveB * rdn:
            blobs = intra_blob(sub_blob, rdn + 1, rng=rng, fig=1, fcr=0)  # -> comp_g
            sub_blob = blobs
...
def cluster_derts(blob, dert__, Ave, fcr, fig):  # analog of frame_to_blobs
    ...
    for y in range(height):  # first and last row are discarded
        ...
        P_ = scan_P_(P_, stack_, blob)    # vertical clustering, adds up_forks per P and down_fork_cnt per stack
