...
def cluster_derts(blob, dert__, Ave, fcr, fig):  # analog of frame_to_blobs
    ...
    for y in range(height):
        P__.append(form_P_(dert__[y, :], crit__[y, :], y))  # horizontal clustering, adds a row of Ps

    P__ = scan_P_(P__)   # vertical clustering, adds P up_forks and down_fork_cnt
    stack_ = form_stack_(P__) # find stack from contiguous Ps
    sub_blob_ = form_blob(stack_) # form blobs from each stack
...
def form_P_(dert_, crit_, y):  # segment dert__ into P__, in horizontal ) vertical order
    ...
    for x in range(x0+1, dert_.shape[0]):  # loop left to right in each row of derts
        ...
        if (~_mask and mask) or sign != _sign:
            ...
            P = dict(I=I, G=G, Dy=Dy, Dx=Dx, M=M, iDy=iDy, iDx=iDx, L=L, x0=x0, sign=_sign, y=y, down_fork_=[], up_fork_=[], stack=[], blob=[])
            P_.append(P)
            # initialize P params:
    ...
    # if L in P = 0, P is the initialized value and we shouldn't pack them
    if P['L'] !=0:
        P_.append(P)

    # i noticed some P is formed with L = 0, L shouldn't be 0 right?
    return P_
...
def form_stack_(P__):
#"""Form stacks from contiguous Ps."""
    ...
    for i in range(len(P__)):
        ...
        for j in range (len(P_)):
            ...
            stack = dict(I=0,G=0,Dy=0,Dx=0,M=0,iDy=0,iDx=0,x0=0,xn=0,y0=0,yn=0,Py_=[],blob=[],sign=[])

            # find contiguous P and form stack from them
            stack = p2stack(stack,P)   
              
            # if stack's P is not empty, store it as new stack
            if len(stack['Py_'])>0 :
                stack_.append(stack) 
...
def p2stack(stack,P): # Used in form_stack_().
            
    # if stack in P is empty
    # if P is belong to any stack, P's stack wouldn't be empty
    if len(P['stack']) == 0:
        
        # store P into stack
        stack['Py_'].append(P)
        # assign stack to P's stack
        P['stack'] = stack
    
        # unpack P
        I,G,Dy,Dx,M,iDy,iDx,L,x0,sign,y,_, _, _,_ = P
        ...
        # if down fork is not empty
        if len(P['down_fork_'])>0 :
            
            # recursively add P and their down fork's P or up fork's P into stack
            stack = p2stack(stack,P['down_fork_'][0])
        
        # if up fork is not empty
        if len(P['up_fork_'])>0 :
            
            # recursively add P and their down fork's P or up fork's P into stack
            stack = p2stack(stack,P['up_fork_'][0])
    
    return stack


def form_blob(stack_):
    
    # initialize blob
    blob = dict(Dert=dict(I=0, G=0, Dy=0, Dx=0, M=0 ,iDy=0,iDx = 0), box=[0,0,0,0], stack_=[], sign=[], dert=[])
    
    for stack in stack_:
       
        # find x0,xn,y0,yn from each Ps
        # update dert with x0,xn,y0,xn
        # repack it back to blob
