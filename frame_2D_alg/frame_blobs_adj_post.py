...
def form_margin(blob_map, diag):  # get 1-pixel margin of blob, orthogonally or orthogonally and diagonally
    
    # get up 1 pixel margin
    up_margin = np.ones_like(blob_map)
    up_margin[:-1,:] = blob_map[1:,:].copy()
    
    # get down 1 pixel margin
    down_margin = np.ones_like(blob_map)
    down_margin[1:,:] = blob_map[:-1,:].copy()
    
    # get left 1 pixel margin
    left_margin = np.ones_like(blob_map)
    left_margin[:,:-1] = blob_map[:,1:].copy()
    
    # get right 1 pixel margin
    right_margin = np.ones_like(blob_map)
    right_margin[:,1:] = blob_map[:,:-1].copy()
    
    # combine all margins
    margin_with_blob_map = ~blob_map + ~up_margin + ~down_margin + ~left_margin + ~right_margin
    # add blob_map to make sure a consistent output in all cases (in the cases whether unmasked area = 1 pixel or >1 pixel)
    # if unmasked area =  1 pixel, the shifted pixel doesn't cover the original unmasked area 
    # if unmasked area > 1 pixel, the shifted pixels will cover partically the original unmasked area
    
    if diag:  # extend by one dert diagonally
        
        # get up and left 1 pixel margin
        upleft_margin = np.ones_like(blob_map)
        upleft_margin[:-1,:-1] = blob_map[1:,1:].copy()
        
        # get up and right 1 pixel margin
        upright_margin = np.ones_like(blob_map)
        upright_margin[:-1,1:] = blob_map[1:,:-1].copy()
        
        # get down and left 1 pixel margin
        downleft_margin = np.ones_like(blob_map)
        downleft_margin[1:,:-1] = blob_map[:-1,1:].copy()
        
        # get down and right 1 pixel margin
        downright_margin = np.ones_like(blob_map)
        downright_margin[1:,1:] = blob_map[:-1,:-1].copy()
        
        # combine all margins
        margin_with_blob_map = margin_with_blob_map + ~upleft_margin + ~upright_margin + ~downleft_margin + ~downright_margin
          
    margin_map = ~(margin_with_blob_map ^ blob_map) # bitwise xor to get the margin only
