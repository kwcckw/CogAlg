...
# unpack list recursively until getting numeric value or z to sum them, inputs must be list
def sum_(L, l, fneg=0):
    if l:
        if L:
            for i, (P, p) in enumerate(zip_longest(L, l, fillvalue=None)):
                if p:
                    if P != None:
