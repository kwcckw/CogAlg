...
class CH(CBase):  # generic derivation hierarchy of variable nesting: extH | derH, their layers and sub-layers
    ...
    def comp_md_C(_md_C, md_C, rn, root, rdn=1., dir=1):
        ...
        return CH(root=root, H=der_md_t, Et=np.append(Et,[rdn, .3 if len(der_md_t)==1 else 2.3]))  # .3 in default comp ext)
            ...
            if blob.G * (len(blob.P_)-1) > ave:  # eval PP
                ...
                if blob.Et[0] * (len(blob.node_)-1)*(blob.rng+1) > ave:
                    ...
                    for PP in blob.node_:
                        vert += PP[3]; lat += PP[4]
                    edge = CG(root_=[frame], node_=blob.node_, latuple=lat, box=[Y,X,0,0], yx=[Y/2,X/2], Et=np.append(blob.Et, [1, blob.n]))  # pack r nd n into et
                    G_ = []
                    for N in edge.node_:  # no comp node_, link_ | PPd_ for now
                        ...
                        if Et[0] > ave:   # no altG until cross-comp
                            PP = CG(fd=0, Et=np.append(Et,[1.,n]),root_=[edge],node_=P_,link_=link_,vert=vert,latuple=lat,box=box,yx=[y,x],n=n)
                            y0,x0,yn,xn = box
    ...
    while True:  # prior vM
        ...
        for Gp in _Gp_:
            ...
            if (dist < max_dist * (radii * icoef**3)
                * (1 + (_G.Et[0] + G.Et[0]) - ave * (_G.Et[3] + G.Et[3]))  # induction
                * (1 + (_G.extH.Et[0] + G.extH.Et[0]) - ave * (_G.extH.Et[3] + G.extH.Et[3]))):  # n should be in Et[2] or Et[3]?
...
def comp_link_(iL_, iEt):  # comp CLs via directional node-mediated link tracing: der+'rng+ in root.link_ rim_t node rims
    ...
    while True:  # xcomp _L_
        ...
        if Et[0] > ave * Et[2] * Med:  # project prior-loop value - new cost
            _L_, ext_Et = set(), np.zeros(4)  # ext_n = 0
            for L in L_:
                        ...
                        for _rev, N in zip((0,1), _L.nodet):
                            ...
                            if len(rim) == med:  # append in comp loop
                                for __L,__rev in rim[0]+rim[1] if fd else rim:
                                    ...
                                    if et[1] * (Et[0]/et[3]/ave) > ave_d * et[2] * Med:  # /__L
            ...
            # refine eval:
            if ext_Et[1] * (Et[0]/Et[3]/ave) > ave_d * ext_Et[2] * Med:  # * ext_n
                med = Med
            else:
                break
        else:
...
def comp_N(_N,N, rn, angle=None, dist=None, dir=1):  # dir if fd, Link.derH=dH, comparand rim+=Link
    ...
    if fd:  # CL
        ...
        Et += et1 + et2; n = 2.3
    Et = np.append(Et, (_N.Et[2] + N.Et[2]) / 2);  Et = np.append(Et, n)  # pack r and n
    # 1st lay:
    ...
    if not fd and _N.altG and N.altG:  # not for CL, eval M?
        alt_Link = comp_N(_N.altG, N.altG, _N.altG.Et[3]/N.altG.Et[3])  # no angle,dist, init alternating PPds | dPs?
...
def sum2graph(root, grapht, fd, nest):  # sum node and link params into graph, aggH in agg+ or player in sub+
    ...
    derH = CH(root=graph)
    for N in node_:
        # graph.n += N.n  # +derH.n, total nested comparable vars (n in Et now)
