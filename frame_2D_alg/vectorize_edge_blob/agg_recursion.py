...
def vectorize_root(blob, verbose):  # vectorization pipeline is 3 composition levels of cross-comp,clustering:
    ...
    for fd, node_ in enumerate(edge.node_t):
        ...
        for PP in node_:  # convert select CPPs to Cgraphs:
            ...
            decHt = reform_dect_(PP.link_) if PP.link_ else [[1],[1]]
...
def reform_dect_(link_):
    ...
    while True:  # unpack lower node_ layer
        ...
        for link in link_:  # get compared Ps and find param maxes
            ...
            for fd in 0, 1:
                if link.roott[fd]:
                    for sub_PP in link.roott[fd].node_t[0] + link.roott[fd].node_t[1]:
...
def sum2graph(root, grapht, fd):  # sum node and link params into graph, aggH in agg+ or player in sub+
    ...
    Gt_,Rim,Val,Rdn = grapht  # right now we are parsing grapht, so we need unpack again
