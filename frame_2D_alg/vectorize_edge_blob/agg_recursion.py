...
def agg_recursion(rroot, root, nrng=1, fagg=0):
    ...
    nrng, Et = rng_recursion(rroot, root, list(combinations(root.node_,r=2)) if fagg else root.link_, Et)  # rng+ appends rim with prelink_
...
def rng_recursion(rroot, root, _links, Et):  # rng++/G_, der+/link_ in sub+, -> rim_H
    ...
    while True:
        for link in _links:
            if isinstance(link, tuple):  # prelink in agg+
                ...
                if nrng==1: fcomp = dist<=ave_dist
                    ...
                    fcomp = M / (dist/ave_dist) > ave * R  # in higher rng, this eval should include rng too?
                if fcomp:
                    G.compared_ += [_G]; _G.compared_ += [G]
                    Link = Clink(node_=[_G,G],distance=dist,angle=[dy,dx], box = copy(G.box))  # always init Link now?
                    Link.box = extend_box(Link.box, _G.box)  # we need box when node is Clink, so sum both nodes' box?
                    comp_G(Link, Et, fd=0)
                    links += [Link]
            else:
                fd = 1  # der+'rng+: directional and node-mediated comp link:
                # should be recursive here? link' node' link' node....
                for G in link.node_:  # search in both directions, G can be link?
                    for _link in G.rim:
                        mA,dA = comp_angle(_link.angle, link.angle)
                        if mA > ave_mA:
                            Link = Clink(node_=[_link,link],distance=1,angle=np.subtract(_link.angle, link.angle),box=copy(link.box))  # always init Link now?
                            Link.box = extend_box(Link.box, _link.box)
                            comp_G(Link, Et, fd=1)  # not existing link?
        ...
        if Et[0] > ave_Gm * Et[2] * nrng:  # rng+ eval per arg cluster because comp is bilateral, 2nd test per new pair
            nrng += 1
            if fd: _links = links  # else reuse combinations?
        else: break  # break
...
def comp_G(link, iEt, fd, nrng=None):  # add dderH to link and link to the rims of comparands, which may be Gs or links

    dderH = CH()  # new layer of link.dderH
    _G, G = link.node_
    if fd:  # G pair is Clink
    ...
    comp_ext(_L,L,_S,S,_A,A, link.distance, dderH)  # this should be for both forks?
    # / G, if >1 PPs | Gs:
    ...
    for i in 0,1:
        ...
        if Val > G_aves[i] * Rdn:
            if fadd:  # to not add the same link twice
                fadd = 0
                for node in _G,G:
                    if fd:  # node is Clink, all mediating links in link.rim should have matching angle:
                        # why we are not checking node.angle? node.rim may empty
...
def sum2graph(root, grapht, fd, nrng):  # sum node and link params into graph, aggH in agg+ or player in sub+
    ...
    for G in G_:
        ...
        if isinstance(G, CG):  # skip summing iderH and latuple if node is link?
