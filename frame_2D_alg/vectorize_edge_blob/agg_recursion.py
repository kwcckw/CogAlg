...
from .classes import add_, CderP, Cgraph, CderG, CderH, Cangle
...
def sum2graph(root, grapht, fd, nrng, fagg):  # sum node and link params into graph, aggH in agg+ or player in sub+
    ...
    valt = add_(valt, evalt); graph.valt = valt
    rdnt = add_(rdnt, erdnt); graph.rdnt = rdnt
    dect = add_(dect, edect); graph.dect = dect
    if fagg:
...
def comp_G(link, Et):
    ...
    Valt = add_(Valt, valt); Rdnt = add_(Rdnt, rdnt); Dect = add_(Dect, dect, 1)
    ...
    for _lev, lev in zip_longest(_Hv.H, Hv.H, fillvalue=0):  # compare common subHs, if lower-der match?
        if _lev and lev:
            if lev[-1] == 0:  # ptuplet
                ...
                DHv.H += [dhv]
                DHv.valt=add_(DHv.valt,dhv.valt); DHv.rdnt=add_(DHv.rdnt ,dhv.rdnt); DHv.dect=add_(DHv.dect,dhv.dect)
            elif lev[-1] == 1:  # derHv
                ...
                DHv.H += [dext]
                DHv.valt = add_(DHv.valt,valt)
                DHv.rdnt = add_(DHv.rdnt,rdnt); DHv.rdnt[0] += valt[1] > valt[0]; DHv.rdnt[1] += valt[0] > valt[1]
                DHv.dect = add_(DHv.dect,dect)
    if DHv.H:
