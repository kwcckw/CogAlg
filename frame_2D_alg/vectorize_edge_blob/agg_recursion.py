...
def agg_recursion(rroot, root, node_, nrng=0, fagg=0):  # compositional agg|sub recursion in root graph, cluster G_
...
def rd_recursion(rroot, root, Q, Et, nrng=1):  # rng+|der+ in G_,link_ -> fork tree, represented in rimH
    ...
    if et[0][fd] > ave_Gm * et[1][fd]:  # single layer accum
        ...
        if not fd:  # der+ is only recursive through sub+ per new graph
            # it's possible for endless recursion if this is true and nrng is already cover all Gs
            ...
            for i in 0,1:
                ...
                for link in rim:
                    ...
                    if dv > ave * dr:
                        ...
                        # both forks share a same rim now?
    ...
    for link in G.rimH[-1] if G.rimH and isinstance(G.rimH[0], list) else G.rimH:  # always rimH?
        if link.daggH:
            ...
            if G.fHH:  # if agg+ adds nesting to node aggH, formed in sub+?
                if len(daggH) > G.extH:  # why this is needed? daggH should be always nested if fHH?
                    dsubH = daggH[-1]
            else:  # from sub+
                dsubH = daggH
...
def comp_G(link, Et, ifd):  # lenH in sub+|rd+, lenHH in agg_compress sub+ only
    ...
    if G.fHH:  # agg++
        ...
        link.daggH += [daggH]  # concat, default (should be flat now since this is only added in sub+, so it's single element before sub+, and may get multiple elements after sub+)
    ...
    for fd, (Val,Rdn,Dec) in enumerate(zip(Valt,Rdnt,Dect)):
        if Val > G_aves[fd] * Rdn:
            ...
            if not ifd:  # drim should be local, but if we didn't pack them, it's actually not needed?
                for G in link.G, link._G:
                    # rim is formed per rd_recursion, converted to rimH in 1st sub+ call? Yes, and add new layer too.
