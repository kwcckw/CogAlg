...
def comp_G(_G, G, distance, A):
    ...
    mval,dval =valt
    Mval += valt[0]; Dval += valt[1]; Mrdn += rdnt[0]+(valt[1]>valt[0]); Drdn += rdnt[1]+(valt[1]<=valt[0])
    # SubH [0]:der_ext, [1]:derLay, [2:]:subH:
    SubH = der_ext +  [[[derLay0], [mval, dval], [mrdn, drdn]]] +  [[dderH,valt,rdnt]]  # i checked and it's not correct to separate their m|d forks, but we need separate the packing of each layer with their valt and rdnt instead
    # / G:
