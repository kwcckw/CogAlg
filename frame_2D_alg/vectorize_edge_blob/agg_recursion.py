...
def feedback(root):  # called from form_graph_, always sub+, append new der layers to root
    ...
    if m+d > sum(G_aves) * (mr+dr):
        ...
        # eval max derH.H[i].node_ as node_ (something like this? But there's a problem where we always have single element in root.derH.H)
        max_index = np.argmax(H.Et[0] for H in root.derH.H)
        max_H = root.derH.H[max_index]
        root.derH.node_ = max.H.node_[:]
