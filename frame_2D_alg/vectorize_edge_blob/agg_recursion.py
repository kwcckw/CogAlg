...
def agg_recursion(rroot, root, node_, nrng=0):  # compositional agg|sub recursion in root graph, cluster G_
    ...
    Et = [[0,0],[0,0],[0,0]]
    for node in node_: node.lenH = -1  # reset node's lenH per rd+?
...
def rd_recursion(rroot, root, Q, Et, nrng=1):  # rng+|der+ in G_,link_ -> fork tree, represented in rim_t
    ...
    if fd:  # der+
        ...
        for link in Q:  # inp_= root.link_, reform links
            if link.Vt[1] > G_aves[1] * link.Rt[1]:  # >rdn incr
                nest_rim(link)  # add nesting before append in comp_G
                comp_G(link, Et)
    ...
    if et[0][fd] > ave_Gm * et[1][fd]:  # single layer accum
        ...
        if fd:
            ...
            # Section below is wrong now
            '''
            pruned_G_ = []
            for G in G_:
                lenHH, lenH = G.lenHH, G.lenH
                if G.rim_t:
                    rim_t = G.rim_t
                    if lenHH >-1:
                    ...
                    if G_lenH > lenH:
                        ...
                        if fd: G.lenH += 1  # increase lenH if fd

            '''
        rd_recursion(rroot, root, link_ if fd else pruned_G_, Et, 0 if fd else nrng+1)
...
def unpack_rim(G, fd):  # rim_t =  [] | rimt|rim_t | rim_tH
    ...
    lenHH = G.lenHH; rim_t = G.rim_t
    rim = []
    if lenHH > -1: rim_t = rim_t[-1]  # nested rimtH | rim_tH  
