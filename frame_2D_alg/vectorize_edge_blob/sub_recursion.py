...
def sub_recursion_eval(root, PP_, fd):  # fork PP_ in PP or blob, no derH,valH,rdnH in blob
    ...
    term = 1
    for PP in PP_:
        # use np sum to get val per fork?
        if np.sum(PP.valT[fd]) > PP_aves[fd] * np.sum(PP.rdnT[fd]) and len(PP.P__) > ave_nsub:  # no select per ptuple
            term = 0
            sub_recursion(PP, fd)  # comp_der|rng in PP -> parLayer, sub_PPs
        elif isinstance(root, CPP):
            root.fb_ += [[[PP.derT[fd]], PP.valT[fd],PP.rdnT[fd]]]  # [derH, valH, rdnH]
...
def sub_recursion(PP, fd):  # evaluate PP for rng+ and der+, add layers to select sub_PPs
    ...
    PP.rdnT[fd][-1][-1][-1] += PP.valT[fd][-1][-1][-1] > PP.valT[1-fd][-1][-1][-1]
    # link Rdn += PP rdn?
    cP__ = [copy(P_) for P_ in P__]
    PP.P__ = form_PP_t(cP__,base_rdn=PP.rdnT[fd][-1][-1][-1])  # P__ = sub_PPm_, sub_PPd_
...
def comp_rng(iP__, rng):  # form new Ps and links in rng+ PP.P__, switch to rng+n to skip clustering?
    ...
    for iP_ in reversed(iP__[:-rng]):  # lower compared row, follow uplinks, no uplinks in last rng rows
        ...
        for P in iP_:
            ...
            mLay, dLay, ValT, RdnT = [[[]]],[[[]]],[[[0]],[[0]]],[[[1]],[[1]]]
            # not sure
            for iderP in P.link_t[0]:  # mlinks
                _P = iderP._P
                for _derP in _P.link_t[0]:  # next layer of mlinks
                    __P = _derP._P  # next layer of Ps
                    comp_P(P,__P, link_,link_m,link_d, [mLay, dLay], ValT, RdnT, fd=0)
            if ValT[0][-1] > P_aves[0] * RdnT[0][-1]:  # not sure
                # add new P in rng+ PP:
                P_ += [CP(ptuple=deepcopy(P.ptuple), derH=[[mLay], [dLay]], dert_=copy(P.dert_), fd=0, box=copy(P.box),
