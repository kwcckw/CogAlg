...
def slice_edge(blob, verbose=False):
    ...
    edge = Cedge(root=blob, node_=[[],[]], box=blob.box, mask__=blob.mask__, derH=['md_',[0,0,1,1,0,0],[]])
    blob.dlayers = [[edge]]
    ...
    while max_:  # queue of (y,x,P)s
        ...
        while maxQue:  # trace max_
            ...
            P = form_P(blob, CP(yx=[y,x], axis=[dy/g, dx/g], cells={(y,x)}, dert_=[(y,x,i,dy,dx,g,ma)], derH=['md_',[0,0,1,1,0,0],[]], link_=[[]]))
            edge.P_ += [P]
            if _P is not None:
                P.link_[0] += [_P]
