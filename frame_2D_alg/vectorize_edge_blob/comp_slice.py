...
def comp_slice(blob, verbose=False):  # high-G, smooth-angle blob, composite dert core param is v_g + iv_ga
    ...
    PPm_,PPd_ = form_PP_t([Pt[0] for Pt in P_], PP_=None, base_rdn=2, ifd=1)  # root fork is 1?
    blob.PPm_, blob.PPd_  = PPm_, PPd_


def comp_P(_P,P, fd=0, ifd=1, derP=None):  #  derP if der+, S if rng+

    aveP = P_aves[fd]
    rn = len(_P.dert_)/ len(P.dert_)

    if fd:  # der+: extend old link derP
        rn *= len(_P.link_ttH[-1][ifd][1]) / len(P.link_ttH[-1][ifd][1])  # derH is summed from links
        ...
        if mval > aveP*mrdn: P.link_ttH[-1][ifd][0] += [derP]  # +ve links, fork selection in form_PP_t
        if dval > aveP*drdn: P.link_ttH[-1][ifd][1] += [derP]
...
def form_PP_t(P_, PP_, base_rdn, ifd):  # form PPs of derP.valt[fd] + connected Ps val

    PP_t = []
    for fd in 0, 1:
        qPP_ = []  # initial sequence_PP s
        for P in P_:
            if not P.root_ttH[-1][ifd][fd]:  # else already packed in qPP
                qPP = [[P]]  # init PP is 2D queue of (P,val)s of all layers?
                P.root_ttH[-1][ifd][fd] = qPP; val = 0
                uplink_ = P.link_ttH[-1][ifd][fd]
                uuplink_ = []  # next layer of uplinks
                while uplink_:  # later uuplink_
                    for derP in uplink_:
                        _P = derP._P
                        if _P not in P_:  # _P is outside of current PP, merge its root PP:
                            PP = P.root_ttH[-2][ifd][fd]  # P.root_ttH[-1] is qPP
                            _PP = _P.root_ttH[-1][ifd][fd]
                            if _PP:  # not None
                                if PP_: PP_.remove(_PP)
                                merge_PP(PP,_PP, fd, ifd)
                        else:
                            _qPP = _P.root_ttH[-1][ifd][fd]
                            if _qPP:
                                if _qPP is not qPP:  # _P may be added to qPP via other downlinked P
                                    val += _qPP[1]  # merge _qPP in qPP:
                                    for qP in _qPP[0]:
                                        qP.root_ttH[-1][ifd][fd] = qPP
                                        qPP[0] += [qP]  # qP_+=[qP]
                                    qPP_.remove(_qPP)
                            else:
                                qPP[0] += [_P]  # pack bottom up
                                _P.root_ttH[-1][ifd][fd] = qPP
                                val += derP.valt[fd]
                                uuplink_ += derP._P.link_ttH[-1][ifd][fd]
                    uplink_ = uuplink_
                    uuplink_ = []
        ...
        rePP_ = reval_PP_(qPP_, fd, ifd)  # PP = [qPP,valt,reval]
        CPP_ = [sum2PP(qPP, base_rdn, fd, ifd) for qPP in rePP_]
...
def reval_PP_(PP_, fd, ifd):  # recursive eval / prune Ps for rePP
    ...
    while PP_:  # init P__
        ...
        if val > ave:
            if reval < ave:  # same graph, skip re-evaluation:
                ...
                rePP = reval_P_(P_,fd,ifd)  # recursive node and link revaluation by med val
                if val > ave:  # min adjusted val
                    rePP_ += [rePP]
                else:
                    for P in rePP: P.root_ttH[-1][ifd][fd] = None  # not sure
        else:  # low-val qPPs are removed
            for P in P_: P.root_ttH[-1][ifd][fd] = None
...
def reval_P_(P_, fd, ifd):  # prune qPP by link_val + mediated link__val

    prune_=[]; Val=0; reval=0  # comb PP value and recursion value

    for P in P_:
        P_val = 0; remove_ = []
        for link in P.link_ttH[-1][ifd][fd]:    # link val + med links val: single mediation layer in comp_slice:
            link_val = link.valt[fd] + sum([mlink.valt[fd] for mlink in link._P.link_ttH[-1][ifd][fd]]) * med_decay
            if link_val < vaves[fd]:
                remove_ += [link]; reval += link_val
            else: P_val += link_val
        for link in remove_:
            P.link_ttH[-1][ifd][fd].remove(link)  # prune weak links
        if P_val * P.rdnt[fd] < vaves[fd]:
            prune_ += [P]
        else:
            Val += P_val * P.rdnt[fd]
    for P in prune_:
        for link in P.link_ttH[-1][ifd][fd]:  # prune direct links only?
            _P = link._P
            _link_ = _P.link_ttH[-1][ifd][fd]
            if link in _link_:
                _link_.remove(link); reval += link.valt[fd]

    if reval > aveB:
        P_, Val, reval = reval_P_(P_, fd, ifd)  # recursion
    return [P_, Val, reval]


def sum2PP(qPP, base_rdn, fd, ifd):  # sum links in Ps and Ps in PP

    P_,_,_ = qPP  # proto-PP is a list
    PP = CPP(fd=fd, node_=P_)
    # accum:
    for i, P in enumerate(P_):
        P.root_ttH[-1][ifd][fd] = PP
        ...
        for derP in P.link_ttH[-1][ifd][fd]:
...
# draft:
def merge_PP(PP, _PP, fd, ifd):

    node_=PP.node_
    for _node in _PP.node_:
        if _node not in node_:
            node_ += [_node]
            _node.root_ttH[-1][ifd][fd] = PP  # reassign root
